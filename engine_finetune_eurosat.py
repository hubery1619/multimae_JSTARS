...
def evaluate(data_loader, model, device):
    ...
    # Dictionary to hold per-class correct predictions and total predictions
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        batch_size = images.shape[0]
        metric_logger.update(loss=loss.item(), acc1=acc1.item(), acc5=acc5.item(), n=batch_size)
    ...
    for class_idx in class_correct.keys():
        print(f'Accuracy of class {class_idx} : {100 * class_correct[class_idx] / class_total[class_idx]:.2f}%')
