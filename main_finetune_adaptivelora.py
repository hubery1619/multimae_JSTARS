...
from util.lora import QkvWithLoRA, QkvWithAdaptiveLoRA
...
def calculate_rank(depth, total_depth, min_rank=8, max_rank=64):
    """Calculate the rank for a layer based on its depth."""
    return min_rank + (max_rank - min_rank) * depth // (total_depth - 2)
...
def main(args):
    ...
    if args.finetune and not args.eval:
        ...
        for i, block in enumerate(model.blocks):
            rank = calculate_rank(i, len(model.blocks))
            if i == len(model.blocks) - 1:
                # Enable full parameter fine-tuning for the last block
                for param in block.parameters():
                    param.requires_grad = True
            else:
                # Dynamically create a new LoRA module with computed rank and initial alpha
                assign_lora = partial(QkvWithLoRA, rank=rank, alpha=1.0)  # initial_alpha set to the middle of the range
                block.attn.qkv = assign_lora(block.attn.qkv)

                # Enable fine-tuning for LoRA parameters
                for param in block.attn.qkv.lora_q.parameters():
                    param.requires_grad = True
                for param in block.attn.qkv.lora_v.parameters():
                    param.requires_grad = True

                # Enable fine-tuning for norm layers
                for param in block.norm1.parameters():
                    param.requires_grad = True
                for param in block.norm2.parameters():
                    param.requires_grad = True

        # Enable fine-tuning for the classifier layer
        ...
        # # Iterate through the blocks in the model
        # for i, block in enumerate(model.blocks):
        #     if i == len(model.blocks) - 1:  # For the last block
        #         # Enable full parameter fine-tuning
        #         for param in block.parameters():
        #             param.requires_grad = True
        #     else:
        #         # For earlier blocks, apply LoRA to self-attention qkv
        #         block.attn.qkv = assign_lora(block.attn.qkv)
        ...
        #         # Enable fine-tuning for norm1 and norm2
        #         for param in block.norm1.parameters():
        #             param.requires_grad = True
        #         for param in block.norm2.parameters():
        #             param.requires_grad = True
    ...
    # Assuming model_without_ddp is defined and distributed if args.distributed is True
    if args.distributed:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu])
        model_without_ddp = model.module

    # Ensure 'head' is a valid attribute and collect its parameters
    if hasattr(model_without_ddp, 'head'):
    ...
    # Collect base parameters excluding head parameters by ID
    base_params = [p for p in model_without_ddp.parameters() if id(p) not in head_param_ids]

    # Initialize default learning rate
    default_lr = args.lr  # Assuming args.lr is your default learning rate

    # Print initial learning rate for head parameters before creating the optimizer
    print(f"Initial Learning Rate for Head Parameters: {default_lr}")

    # Setting parameter groups
    param_groups = [
        {'params': base_params},
        {'params': head_params, 'lr': args.lr * 10} if head_params else {'params': []}
    ]

    # Create the optimizer with AdamW
    optimizer = torch.optim.AdamW(param_groups, lr=args.lr)

    # Print modified learning rate for head parameters after creating the optimizer
    if head_params:
        print(f"Modified Learning Rate for Head Parameters: {optimizer.param_groups[1]['lr']}")
    else:
        print("No head parameters found or no special learning rate set.")
