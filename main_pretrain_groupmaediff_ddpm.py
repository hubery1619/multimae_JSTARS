...
import models_mae_diffusion
import models_mae_diffusion_integrate
import models_groupcrossmae_diffusion_splitself
import models_groupcrossmae_diffusion_ddpm

from engine_pretrain_diffusion_ddpm import train_one_epoch, train_one_epoch_temporal
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model_type', default=None, choices=['group_c', 'temporal', 'vanilla', 'group_cross', 'MAE_diffusion', 'MAE_diffusion_integrate', 'group_diffusion', 'groupmaediff_ddpm'],
...
def main(args):
    ...
    elif args.model_type == 'MAE_diffusion':
        model = models_mae_diffusion.__dict__[args.model](img_size=args.input_size,
                                                patch_size=args.patch_size,
                                                in_chans=dataset_train.in_c,
                                                norm_pix_loss=args.norm_pix_loss)
        
    elif args.model_type == 'MAE_diffusion_integrate':
        model = models_mae_diffusion_integrate.__dict__[args.model](img_size=args.input_size,
                                                patch_size=args.patch_size,
                                                in_chans=dataset_train.in_c,
                                                norm_pix_loss=args.norm_pix_loss)

    elif args.model_type == 'groupmaediff_ddpm':
        model = models_groupcrossmae_diffusion_ddpm.__dict__[args.model](img_size=args.input_size,
                                                patch_size=args.patch_size,
                                                in_chans=dataset_train.in_c,
                                                norm_pix_loss=args.norm_pix_loss)
    ...
    score_model = sde.ScoreModel(model.decoder_diffusion, pred='x0_pred', sde=sde.VPSDE())
    ...
    if global_rank == 0 and args.wandb is not None:
        wandb.init(project=args.wandb, entity="huberyliu")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.model_type == 'temporal':
            ...
            train_stats = train_one_epoch(
                model, score_model, data_loader_train,
                optimizer, device, epoch, loss_scaler,
                log_writer=log_writer,
                args=args
            )
