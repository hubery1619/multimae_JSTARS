...
class MultiMaskedAutoencoderGroupChannelViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, spatial_mask=False,
        ...
        self.decoder_selfblocks = nn.ModuleList([
            Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer)
            for i in range(decoder_depth)])

        self.decoder_crossblocks = nn.ModuleList([
    ...
    def forward_decoder(self, x_feat, x_encoder, mask, mask_partial, ids_restore):
        ...
        # for i, blk in enumerate(self.decoder_crossblocks):
        #     if self.weight_fm:
        ...
        for i, (cross_blk, self_blk) in enumerate(zip(self.decoder_crossblocks, self.decoder_selfblocks)):
            # run crossblock
            if self.weight_fm:
                x_reconstruct = cross_blk(x_mask_partial_reshaped, self.dec_norms[i](y[..., i]))
            else:
                x_reconstruct = cross_blk(x_mask_partial_reshaped, x_encoder_w_class)

            # run selfblock
            x_reconstruct = self_blk(x_reconstruct)
    ...
    def forward_loss(self, imgs, pred, mask, mask_partial):
        ...
        # 使用torch.index_select对predict进行通道重排
...
def multimae_vit_base_patch16_dec512d8b(**kwargs):
    model = MultiMaskedAutoencoderGroupChannelViT(
...
def multimae_vit_large_patch16_dec512d8b(**kwargs):
    model = MultiMaskedAutoencoderGroupChannelViT(
        channel_embed=256, embed_dim=1024, depth=24, num_heads=16,
        decoder_channel_embed=128, decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
def multimae_vit_huge_patch14_dec512d8b(**kwargs):
    model = MultiMaskedAutoencoderGroupChannelViT(
...
multimae_vit_base_patch16 = multimae_vit_base_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
multimae_vit_large_patch16 = multimae_vit_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
multimae_vit_huge_patch14 = multimae_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 blocks
