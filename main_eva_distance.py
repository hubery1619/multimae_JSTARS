...
patches_per_side = 12  # 12 patches per side in one group
...
def compute_distance_matrix(patch_size, patches_per_side, num_groups):
    ...
    for i in range(total_patches):
        for j in range(total_patches):
            group_i, index_i = divmod(i, num_patches_per_group)
            group_j, index_j = divmod(j, num_patches_per_group)
            xi, yi = divmod(index_i, patches_per_side)
            xj, yj = divmod(index_j, patches_per_side)

            if group_i == group_j or group_i != group_j:  # Same logic applies regardless of group
...
def main(args):
    ...
    #### one batch #############
    distances = [AverageMeter() for _ in range(24)]
    ...
    for idx, (xs, _) in enumerate(data_loader_train):
        ...
        for i, attn in enumerate([attn for attn in attns if attn is not None]):
            ...
            distance = calculate_mean_attention_dist(patch_size, attn, patches_per_side, num_groups)
    ...
    ax.plot(range(0, 24), distances, marker="o")
    ax.set_xlabel("Depth")
