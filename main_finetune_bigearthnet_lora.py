...
from engine_finetune_bigearthnet import (train_one_epoch,
                             evaluate
                             )
...
def get_args_parser():
    ...
    parser.add_argument('--model_type', default=None, choices=['group_c', 'resnet', 'resnet_pre',
                                                               'temporal', 'vanilla', 'bigearthnet_finetune'],
    ...
    parser.add_argument('--test_path', default='/home/val_62classes.csv', type=str,
                        help='Test .csv path')
    parser.add_argument('--dataset_type', default='rgb', choices=['rgb', 'temporal', 'sentinel', 'euro_sat', 'naip', 'bigearthnet_finetune'],
...
def random_subset(dataset, fraction, seed=42):
...
def main(args):
    ...
    train_frac = getattr(args, "train_frac", 0.1)  
    val_frac = getattr(args, "val_frac", 1.0)  

    if train_frac < 1.0:
        dataset_train = random_subset(dataset_train, train_frac, seed=42)

    if val_frac < 1.0:
        dataset_val = random_subset(dataset_val, val_frac, seed=42)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (epoch % args.save_every == 10 or epoch + 1 == args.epochs):
            misc.save_model(
        ...
        if args.dataset_type == 'bigearthnet_finetune':
            if epoch >= 0:
                print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['mAP']:.1f}%")
