...
def main(args):
    ...
    #### one batch #############
    nmis = [AverageMeter() for _ in range(24)]

    for idx, (xs, _) in enumerate(data_loader_train):
        xs = xs.cuda()
        ...
        for i, attn in enumerate([attn for attn in attns if attn is not None]):
            attn = attn[:,:,1:145,1:145]  # drop cls token
            attn = attn + 1e-8
    ...
    nmis = [torch.mean(nmi.avg) for nmi in nmis]
        # 将数据写入到文本文件
    with open('crossattention_nofm_nmi_group1.txt', 'w') as f:
        for nmi in nmis:
            f.write(f"{nmi}\n")
    ax.plot(range(0, 24), nmis, marker="o")
    ax.set_xlabel("Depth")
    ax.set_ylabel("Normalized MI")
    ax.set_ylim(top=0.6, bottom=0.0)
    plt.savefig('normalized_train_self_attention_nofm_group1.png', dpi=150)  # 可以调整dpi以改变输出图像的质量


    # # 检查是否有多个GPU可用，如果有，使用DataParallel
    # if torch.cuda.device_count() > 1:
    #     print(f"Using {torch.cuda.device_count()} GPUs!")
    #     model = nn.DataParallel(model)

    # model.cuda()  # 把模型发送到默认的设备
    # nmis = [AverageMeter() for _ in range(24)]

    # # 将tqdm添加到循环中以显示进度条
    # for idx, (xs, _) in tqdm(enumerate(data_loader_val), total=len(data_loader_val), desc="Processing"):
    #     xs = xs.cuda()
    ...
    #         nmi = calculate_nmi(attn)
    #         nmis[i].update(torch.mean(nmi, dim=0), n=xs.size(0))  # average w.r.t. batch

    # fig, ax = plt.subplots(1, 1, figsize=(4.3, 4), dpi=150)
    # nmis_means = [nmi.average() for nmi in nmis]
    # ax.plot(range(24), nmis_means, marker="o")
    # ax.set_xlabel("Attention Head")
    # ax.set_ylabel("Normalized Mutual Information")
    # ax.set_ylim(top=0.6, bottom=0.0)
    # plt.savefig('normalized_self_attention_mi_plot_oneepoch.png', dpi=150)
